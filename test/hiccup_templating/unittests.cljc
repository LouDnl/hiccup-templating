  ...
  (:require
   ...
   #?(:clj  [hiccup-templating.core
              :refer [from-file from-stream from-string
                      ...
                      template-to-hiccup]]
      :cljs [hiccup-templating.core
              :refer [from-string as-xhtml-string as-xhtml-string-escaped
                      template-to-hiccup]])
...
#?(:clj (deftest from-file-validatepath-allows-legit-file
          (let [t (hiccup-templating.core.reader/template-from-edn-file
                   test-path test-template :validatepath true)]
            (is (map? t)))))

#?(:clj (deftest from-file-validatepath-blocks-filename-traversal
          (is (thrown-with-msg?
...
#?(:clj (deftest from-file-validatepath-blocks-path-traversal
          (is (thrown-with-msg?
               clojure.lang.ExceptionInfo #"`\.\.` segments are not allowed"
               (hiccup-templating.core.reader/template-from-edn-file
                "test/resources/../../.." "deps.edn" :validatepath true)))))
...
#?(:clj (deftest from-stream-rejects-oversize-input
          (is (thrown-with-msg?
...
#?(:clj (deftest as-xhtml-string-renders-doctype-and-body
          (let [html (as-xhtml-string [:html [:body [:p "hi"]]])]
            (is (string? html))
            (is (re-find #"<!DOCTYPE html" html))
            (is (re-find #"<p>hi</p>" html)))))

#?(:clj (deftest as-xhtml-stream-writes-same-bytes-as-string
          (let [tree [:html [:body [:p "hi"]]]]
            (is (= (as-xhtml-string tree)
                   (.toString (as-xhtml-stream tree)))))))

#?(:clj (deftest as-xhtml-string-currently-does-not-escape-values
          ...
          (let [payload "<script>alert(1)</script>"
            ...
            (is (clojure.string/includes? html payload)))))

#?(:clj (deftest as-xhtml-string-escaped-escapes-values
          (let [payload "<script>alert(1)</script>"
                html    (as-xhtml-string-escaped
                         (render "{:template [:div :data/x]}" {:x payload}))]
            (is (not (clojure.string/includes? html payload)))
            (is (clojure.string/includes? html "&lt;script&gt;")))))

#?(:clj (deftest as-xhtml-stream-escaped-matches-string-escaped
          (let [tree (render "{:template [:div :data/x]}" {:x "<b>hi</b>"})]
            (is (= (as-xhtml-string-escaped tree)
                   (.toString (as-xhtml-stream-escaped tree)))))))

#?(:clj (deftest as-xhtml-stream-returns-writable-baos
...
#?(:clj (deftest sample-template-produces-expected-tree
...
#?(:clj (deftest sample-template-sub-template-uses-or-fallback
          (let [template (from-file test-path test-template)
                hiccup   (template-to-hiccup template sample-data
                                             :template-key :sub-template)]
            (is (= [:footer [:div "Goodbye now, come again!"]] hiccup)))))

#?(:clj (deftest resources-exist-on-disk
